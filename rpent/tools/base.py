...
class Tool(Generic[ParamsT]):
    ...
    readonly skips automatic observation capture; parallel allows shared admission.
    """
...
def readonly(handler: Callable[ParamsT, ToolResult]) -> Callable[ParamsT, ToolResult]:
    ...
    Place this marker below @tool. It does not enable concurrent execution.
    """
    setattr(handler, "_rpent_readonly", True)
    return handler


def parallel(handler: Callable[ParamsT, ToolResult]) -> Callable[ParamsT, ToolResult]:
    """Allow shared admission with other parallel tools, excluding exclusive calls.

    Place this marker below @tool. It does not change observation capture.
    """
    setattr(handler, "_rpent_parallel", True)
    return handler
...
def tool(function: Callable[ParamsT, ToolResult], /) -> Tool[ParamsT]:
    ...
    Place @tool above the policy markers. By default calls are exclusive and
    capture observations afterward; @readonly disables capture, while @parallel
    independently allows shared admission.
    """
    ...
    return Tool(
        ...
        parallel=getattr(function, "_rpent_parallel", False),
    )

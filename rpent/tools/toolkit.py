...
class _Scheduler:
    """Run parallel tools together and exclusive tools one at a time.
    ...
    def _can_start(self, call: _Call) -> bool:
        first_exclusive = next(
            (pending for pending in self._pending if not pending.tool.parallel), None
        )

        if not call.tool.parallel:
            return not self._active_calls and call is first_exclusive

        return first_exclusive is None and all(
            active.tool.parallel for active in self._active_calls
        )

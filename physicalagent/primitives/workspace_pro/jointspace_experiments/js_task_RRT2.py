"""task_t0 (open BOTTOM drawer, default cabinet) SOLVE. The low bar + bowl/plate clutter make
...
qsol=None
seeds=[qhome]+[np.clip(qhome+np.random.uniform(-0.6,0.6,7),JL[:,0],JL[:,1]) for _ in range(40)]
for hz in [0.952,0.960,0.946,0.968]:
    for sd in seeds:
...
if qsol is None:
    ...
    imageio.mimsave(f"{OUT}/task_t0_SOLVE.mp4", frames, fps=30, quality=8); rend.close(); env.close(); sys.exit()
...
if not path:
    imageio.mimsave(f"{OUT}/task_t0_SOLVE.mp4", frames, fps=30, quality=8); rend.close(); print("RRT FAILED"); env.close(); sys.exit()
...
imageio.mimsave(f"{OUT}/task_t0_SOLVE.mp4", frames, fps=30, quality=8); rend.close()

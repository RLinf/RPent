...
- Interactive driver: `physicalagent/primitives/interactive_driver.py` — REPL via `$OUTPUT_DIR/repl/hybrid_repl_<tag>/command.json` polling; supports move_to/pi0_pick/release/set_gripper/reset/exit actions; dumps state_<N>.json + image_<N>.png after every command.

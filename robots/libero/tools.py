...
def _step_env(ctx: ToolContext[LiberoRuntime], action) -> None:
    ...
    ctx.check_cancelled()
    runtime = ctx.robot
    obs, reward, terminated, truncated, _info = runtime.env.step(action)
    if runtime._flywheel is not None:
        runtime._flywheel.add_transition(action, obs, reward, terminated, truncated)
...
def _vlm_chunk(ctx: ToolContext[LiberoRuntime], instruction: str):
    ...
    try:
        ...
        vla_id = (
            runtime._flywheel.add_proposal(instruction, actions)
            if runtime._flywheel is not None
            else -1
        )
        chunk_obs, rewards, terminated, truncated, _info = runtime.env.chunk_step(
            actions, return_all_frames=True
        )
        for index, obs in enumerate(chunk_obs):
            ctx.record_frame(obs["main_images"])
            if runtime._flywheel is not None:
                runtime._flywheel.add_transition(
                    actions[index],
                    obs,
                    rewards[index],
                    terminated[index],
        ...
        runtime.executed_steps += int(np.asarray(terminated).size)
...
def segment(
    ...
    *,
    ...
    """SAM3 visual segmentation over an existing run artifact. Provide exactly one text prompt or single positive point. A successful top-ranked mask is projected through the matching world map to produce world_xyz.

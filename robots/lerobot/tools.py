...
* :class:`LerobotPrimitives` — the primitive driver the toolkit owns. Holds
    the env client and exposes one method per primitive tool.
...
from robots.lerobot.geometry import robust_surface_centroid
...
class LerobotPrimitives:
    ...
    def __init__(self, env: LerobotEnvClient):
        self.env = env
...
def view_env_state(
    ...
    *,
    ...
    """Return one recorded state with its scene and arm camera images."""
    try:
    ...
    for slot, name in image_slots.items():
        if name not in record.artifacts:
            continue
        try:
            out[slot] = state.load_bytes(name, step=record.step_idx)

...
def get_env_spec() -> EnvSpec:
    ...
    :func:`get_toolkit`). The runner hooks keep CLI and dashboard orchestration
    independent of the concrete robot implementation.
    """
    return EnvSpec(
        name="lerobot",
...
def get_toolkit(
    *,
    ...
    env RPC stub (``{"env": LerobotEnvClient(...)}``).
    """
...
def _init_shared_runtime(
    ...
    """Return the empty session-owned runtime for the standalone SO101 env."""
    del args, output_dir, dashboard_events
    return [], {}
...
def _init_task_runtime(
    ...
    """Initialize the task-owned SO101 environment runtime."""
    return _init_runtime(args, output_dir, dashboard_events)

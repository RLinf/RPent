...
class RoboCasaPrimitives:
    ...
    def _resolve_grip(self, gripper, target_q):
        ...
        if isinstance(gripper, str) or gripper is None:
            ...
            return float(np.clip(60.0 * (cur - target_q), -1.0, 1.0))   # +a[6] closes (qpos↓)
    ...
    def move_to(self, xyz, gripper="hold", step_clip=0.02, max_steps=200, tol=0.012):
        ...
        for i in range(max_steps):
            ...
            if dist < tol:
                return {"ok": True, "steps": i, "final_dist": dist, "eef": cur.tolist(),
    ...
    def move_delta(self, dxyz, gripper="hold", step_clip=0.02, max_steps=80):
        self._vla_desync = True
        return self.move_to(self.env.eef_pos + np.asarray(dxyz), gripper, step_clip, max_steps)
    ...
    def move_base(self, forward=0, lateral=0, turn=0, steps=10, gripper="hold"):
        ...
        a = self._zero(base_mode=1.0)
        a[7:10] = [np.clip(forward, -1, 1), np.clip(lateral, -1, 1), np.clip(turn, -1, 1)]
    ...
    def _yaw(self):
        ...
        return float(R.from_quat(np.asarray(self.env.current_raw_obs["robot0_base_quat"])).as_euler("xyz")[2])
    ...
    def navigate_to(self, xy, tol=0.20, max_steps=300, gripper="hold"):
        ...
        for i in range(max_steps):
            ...
            if dist < tol:
                ...
                return {"ok": True, "steps": i, "final_dist": dist, "moved": moved,
                        "start_pos": start.tolist(), "base_pos": bp.tolist()}
        ...
        return {"ok": False, "steps": max_steps, "final_dist": float(np.linalg.norm(target - bp[:2])),
                "moved": moved, "stuck": moved < 0.12, "start_pos": start.tolist(),
                "base_pos": bp.tolist()}
    ...
    def run_rldx_skill(self, base_clip, max_chunks, use_prompt, prompt, force_reset,
        ...
        return self._rldx.run(task_lang, max_chunks, n_action_steps,
                              base_clip=base_clip,
    ...
    def reset(self):
        ...
        if not self._allow_reset:
            return {"error": "reset is DISABLED in this run (no-reset/matched evaluation). "
    ...
    def rldx_skill(self, base_clip=None,
        ...
        return self.run_rldx_skill(base_clip, max_chunks, use_prompt, prompt, force_reset,
                                   n_action_steps, settle_patience, settle_eps)

    def rldx_arm(self, base_clip=0.1,
                 max_chunks=int(os.environ.get("RLDX_MAX_CHUNKS", 70)),
                 use_prompt=None, prompt="", force_reset=False,
                 n_action_steps=8,
                 settle_patience=int(os.environ.get("RLDX_SETTLE_PATIENCE", 999)),

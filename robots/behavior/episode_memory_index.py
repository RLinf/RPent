...
class EpisodeFrameKey:
    ...
    def __post_init__(self) -> None:
        for field_name in ("frame_id", "episode_id", "experience_id", "task_name", "keyframe_kind", "source_record_id"):
        ...
        if isinstance(self.frame_index, bool) or self.frame_index < 0:
            fail("MEMORY_EPISODE_SCHEMA_INVALID", "frame.frame_index", "expected non-negative int")
        if isinstance(self.embedding_row, bool) or self.embedding_row < 0:
            fail("MEMORY_EPISODE_SCHEMA_INVALID", "frame.embedding_row", "expected non-negative int")
...
class EpisodeExperience:
    ...
    def __post_init__(self) -> None:
        for field_name in ("episode_id", "experience_id", "logical_experience_id", "task_name"):
            _nonempty_string(getattr(self, field_name), path=f"experience.{field_name}")
        if not self.frame_keys:
            fail("MEMORY_EPISODE_SCHEMA_INVALID", "experience.frame_keys", "at least one head keyframe required")
        for frame in self.frame_keys:
            if frame.episode_id != self.episode_id or frame.experience_id != self.experience_id or frame.task_name != self.task_name:
                fail("MEMORY_EPISODE_SCHEMA_INVALID", "experience.frame_keys", "frame identity does not match experience")
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "canonical_trajectory_ref": None if self.canonical_trajectory_ref is None else dict(self.canonical_trajectory_ref),
            "trajectory_refs": [dict(item) for item in self.trajectory_refs],
            "reproduction_evidence": [dict(item) for item in self.reproduction_evidence],
    ...
    def from_mapping(cls, value: Mapping[str, Any]) -> "EpisodeExperience":
        ...
        if value["schema_id"] != SCHEMA_ID:
            fail("MEMORY_EPISODE_SCHEMA_INVALID", "experience.schema_id", "schema mismatch")
        frame_values = value["frame_keys"]
        if not isinstance(frame_values, list):
            fail("MEMORY_EPISODE_SCHEMA_INVALID", "experience.frame_keys", "expected list")
        return cls(
            episode_id=str(value["episode_id"]),
            experience_id=str(value["experience_id"]),
            ...
            canonical_trajectory_ref=None if value["canonical_trajectory_ref"] is None else dict(value["canonical_trajectory_ref"]),
            trajectory_refs=tuple(dict(item) for item in value["trajectory_refs"]),
            reproduction_evidence=tuple(dict(item) for item in value["reproduction_evidence"]),
            source=dict(value["source"]),
            metadata=dict(value["metadata"]),
            frame_keys=tuple(EpisodeFrameKey.from_mapping(item) for item in frame_values),
        )
...
class EpisodeMemoryIndex:
    def __init__(
        ...
        *,
        ...
        self._frames = tuple(frame for exp in self._experiences for frame in exp.frame_keys)
        self._head = l2_matrix(head_embeddings, path="head_embeddings")
        if self._head.shape[0] != len(self._frames):
            fail("MEMORY_EPISODE_INDEX_INVALID", "head_embeddings", "row count must equal head keyframes")
        self._experience_by_id = {item.experience_id: item for item in self._experiences}
        self._experience_by_episode = {item.episode_id: item for item in self._experiences}
        if len(self._experience_by_id) != len(self._experiences) or len(self._experience_by_episode) != len(self._experiences):
            fail("MEMORY_EPISODE_INDEX_INVALID", "experiences", "experience and episode IDs must be unique")
        by_task: dict[str, list[int]] = {}
        for index, frame in enumerate(self._frames):
            if frame.embedding_row != index:
                fail("MEMORY_EPISODE_INDEX_INVALID", "frames", "embedding rows must be contiguous")
        ...
        for channel, values in (wrist_shadow_embeddings or {}).items():
            ...
            if name not in SHADOW_CHANNELS:
                fail("MEMORY_EPISODE_INDEX_INVALID", f"shadow.{name}", "only wrist shadow channels are accepted")
            matrix = l2_matrix(values, path=f"shadow.{name}")
            if matrix.shape[0] != len(self._frames):
                fail("MEMORY_EPISODE_INDEX_INVALID", f"shadow.{name}", "row count mismatch")
            shadow[name] = matrix
    ...
    def search(
        ...
        *,
        ...
        distances = np.asarray(1.0 - np.clip(query @ self._head[list(candidates)].T, -1.0, 1.0), dtype=np.float64)[0]
        best_by_experience: dict[str, tuple[float, EpisodeFrameKey, dict[str, float]]] = {}
        for offset, frame_index in enumerate(candidates):
            frame = self._frames[frame_index]
            shadow_distances = self._shadow_distances(frame_index, wrist_shadow_embeddings)
            candidate = (float(distances[offset]), frame, shadow_distances)
            current = best_by_experience.get(frame.experience_id)
            if current is None or (candidate[0], frame.frame_id) < (current[0], current[1].frame_id):
        ...
        ordered = sorted(hits, key=lambda hit: (hit.distance, hit.experience.experience_id, hit.matched_frame.frame_id))
        return tuple(
            EpisodeMemoryHit(
                rank=index,
    ...
    def retrieve(
        ...
        *,
        ...
        selected = next((hit for hit in hits if hit.distance <= HEAD_ACTIVE_DISTANCE_MAX), None)
        return MappingProxyType(
            {
                "schema_id": "rpent_behavior_episode_memory_retrieval_v1",
                "decision": "use_experience" if selected is not None else "record_new",
                "reason": "head_keyframe_under_active_threshold" if selected is not None else "no_same_task_head_keyframe_under_active_threshold",
    ...
    def _shadow_distances(
        ...
        for channel, query in (queries or {}).items():
            ...
            result[name] = float(1.0 - np.clip(row @ self._shadow[name][frame_index : frame_index + 1].T, -1.0, 1.0)[0, 0])
        return result
...
def load_current_catalog(memory_dir: Path | None) -> EpisodeMemoryIndex:
    ...
    if not root.is_dir():
        fail("MEMORY_EPISODE_CATALOG_MISSING", str(root), "explicit memory-dir is missing")
    pointer_path = root / "current.json"
    pointer = _read_json(pointer_path)
    require_exact_keys(pointer, {"schema_id", "revision_document_sha256"}, path="current.json")
    if pointer["schema_id"] != CURRENT_POINTER_SCHEMA_ID:
        fail("MEMORY_EPISODE_POINTER_INVALID", "current.json", "schema mismatch")
    revision_sha = require_sha256(pointer["revision_document_sha256"], path="current.revision_document_sha256")
    revision_dir = root / "revisions" / revision_sha
    return load_revision_dir(revision_dir, expected_revision_sha256=revision_sha)


def load_revision_dir(revision_dir: Path, *, expected_revision_sha256: str | None = None) -> EpisodeMemoryIndex:
    if not revision_dir.is_dir():
        fail("MEMORY_EPISODE_REVISION_MISSING", str(revision_dir), "revision directory missing")
    ...
    revision_sha = require_sha256(manifest["revision_document_sha256"], path="manifest.revision_document_sha256")
    if expected_revision_sha256 is not None and revision_sha != expected_revision_sha256:
        fail("MEMORY_EPISODE_HASH_MISMATCH", "manifest.revision_document_sha256", "current pointer mismatch")
    revision_bytes = _read_regular(revision_dir / "revision.json")
    if sha256_bytes(revision_bytes) != revision_sha:
        fail("MEMORY_EPISODE_HASH_MISMATCH", "revision.json", "document digest mismatch")
    catalog_bytes = _read_regular(revision_dir / "catalog.jsonl")
    if sha256_bytes(catalog_bytes) != require_sha256(manifest["catalog_sha256"], path="manifest.catalog_sha256"):
    ...
    if sha256_bytes(embeddings_bytes) != require_sha256(manifest["embeddings_npz_sha256"], path="manifest.embeddings_npz_sha256"):
        fail("MEMORY_EPISODE_HASH_MISMATCH", "embeddings.npz", "embedding digest mismatch")
    ...
    index = EpisodeMemoryIndex(experiences=experiences, head_embeddings=head, wrist_shadow_embeddings=shadow, revision=revision)
    if index.episode_count != int(manifest["experience_count"]) or index.frame_count != int(manifest["frame_count"]):
...
def write_candidate_revision(
    *,
    ...
    embedding_payload = _npz_bytes({"head": candidate_index._head, **dict(candidate_index._shadow)})
    ...
    pointer = {"schema_id": CURRENT_POINTER_SCHEMA_ID, "revision_document_sha256": revision_sha}
    if activate_current:
        _atomic_write(root / "current.json", canonical_json_file_bytes(pointer, path="current"))
    return MappingProxyType({"revision_document_sha256": revision_sha, "revision_dir": str(revision_dir), "current": bool(activate_current)})
    ...
    return MappingProxyType(
        {
            ...
            "decision": "append_reproduction_evidence" if accepted else "record_new_experience",
            "reason": "same_task_bidirectional_95pct_keyframe_coverage" if accepted else "coverage_below_threshold",
            ...
            "logical_experience_id": existing.logical_experience_id if accepted else candidate.logical_experience_id,
            "canonical_trajectory_ref": None if existing.canonical_trajectory_ref is None else dict(existing.canonical_trajectory_ref),
...
def keyframe_coverage(query_embeddings: np.ndarray, catalog_embeddings: np.ndarray) -> float:
...
def _npz_bytes(arrays: Mapping[str, np.ndarray]) -> bytes:
    with io.BytesIO() as buffer:
        np.savez(buffer, **{name: np.asarray(value, dtype=np.float32) for name, value in arrays.items()})
...
def _write_revision_dir(
    ...
    *,
    ...
    _write_new(revision_dir / "manifest.json", canonical_json_file_bytes(dict(manifest), path="manifest"))

    ...
    try:
        while True:
            try:
                ...
                break
            if cmd == "step":
                # DIAGNOSTIC: catch any exception from env.step (incl. C-level
                # crashes that bubble up as RuntimeError) and dump full
                # context to stderr before propagating. Without this the
                # parent process sees only EOFError and we lose the cause.
                try:
                    env_return = env.step(data)
                except Exception as _e:
                    import traceback, sys
                    print(f"\n[worker] env.step crashed: {type(_e).__name__}: {_e}",

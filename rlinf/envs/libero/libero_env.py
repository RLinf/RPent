...
class LiberoEnv(gym.Env):
    ...
    def get_env_fn_params(self, env_idx=None):
        ...
        for env_id in range(self.num_envs):
            ...
            override = os.environ.get("LIBERO_BDDL_OVERRIDE")
            if override:
                final_path = override
    ...
    def _extract_image_and_state(self, obs):
        # When set_image_render_enabled(False) is in effect, robosuite drops
        # the image observables from obs. Substitute the most recent rendered
        # frames so downstream code sees the cached views; the per-step EGL
        # render is skipped, which is the whole point of the toggle.
        if "agentview_image" in obs:
            full_image = get_libero_image(obs)
            self._cached_full_image = full_image
        else:
            full_image = getattr(self, "_cached_full_image", None)
            if full_image is None:
                # First step ever without rendering — fall back to a zero
                # frame so dtype/shape downstream stays consistent.
                full_image = np.zeros((128, 128, 3), dtype=np.uint8)
        if "robot0_eye_in_hand_image" in obs:
            wrist_image = get_libero_wrist_image(obs)
            self._cached_wrist_image = wrist_image
        else:
            wrist_image = getattr(self, "_cached_wrist_image", None)
    ...
    def set_image_render_enabled(self, enabled: bool,
                                  observable_names=("agentview_image",
                                                    "robot0_eye_in_hand_image",
                                                    "agentview_depth",
                                                    "robot0_eye_in_hand_depth")):
        """Toggle camera-image observables in robosuite to skip EGL rendering
        during pure OSC primitives. Call with enabled=False before long
        scripted move_to/release/articulate loops; flip back to True before
        any Pi0 invocation (Pi0 needs main_images).

        Robosuite renders the camera once per env.step when the observable is
        enabled. Disabling it cuts the per-step EGL load to zero. The cached
        `agentview_image` is left stale until the next render, so a primitive
        that needs a fresh image should re-enable and call env.step once."""
        for w in self.env.workers:
            for name in observable_names:
                w.env_call(
                    method="modify_observable",
                    args=[name, "enabled", bool(enabled)],
                    target="robosuite",
                )

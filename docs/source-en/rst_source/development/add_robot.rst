...
- Implement ``_capture_observation(*, command, result, elapsed_s)`` using
  ...
  this after every non-readonly handler, including failed handlers.
...
- ``output_dir`` is the runner-created working directory. Environment artifacts
  ...
  run directory.
- ``@readonly`` skips automatic observation capture; ``@parallel`` separately
  enables shared execution. Both markers go below ``@tool``. Keep actions
  exclusive and call ``ctx.check_cancelled()`` at safe boundaries in long loops.

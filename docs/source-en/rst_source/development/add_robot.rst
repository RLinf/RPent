...
For a new robot named ``myenv``, use the following directory layout:
...
``robot_spec.init_runtime(args, output_dir)`` to initialize the robot and
VLA services and build the toolkit inputs. The robot can spawn as many

...
* ``robots/franka/config/robot_config.yaml`` contains only the machine identity (robot
	IP, camera serials, gripper) and workspace geometry (target/reset poses and
	safety limits).
* ``robots/franka/runtime_config.py`` holds the developer defaults (primitive-control
...
* Hand-eye calibration is generated by ``easy_handeye`` (see the Calibration
  ...
  lives at ``tests/robots/franka/fixtures/hand_eye_calibration.json``.

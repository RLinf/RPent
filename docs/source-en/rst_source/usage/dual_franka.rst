...
* ``robots/dual_franka/config/robot_config.yaml`` contains only the machine identity
...
* ``robots/dual_franka/runtime_config.py`` holds the developer defaults (primitive
	control, perception tuning, episode length, node placement).
* Hand-eye calibration is generated by ``easy_handeye`` (see the Calibration
  section below) rather than checked in; a sample bundle for offline tests
  lives at ``tests/robots/dual_franka/fixtures/hand_eye_calibration.json``.
  Camera intrinsics are captured at runtime.
...
localization bounds and base-frame transform live in ``config/robot_config.yaml``'s

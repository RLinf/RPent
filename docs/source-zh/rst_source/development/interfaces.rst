...
.. list-table::
   ...
   * - ``init_runtime``
     - 启动或连接全部 runtime components，或只处理指定名称的子集，并构造对应的
       ...
       的 shared 和 unique 子集后分别传入。``DashboardEventSink`` 用于上报运行\
       时状态。
...
约定：用 ``toolkit.get_tools_spec()`` 把工具交给模型；每次调用
``toolkit.execute_tool(name, input_dict)``；把结果喂回模型；在 ``finish`` 工具\
或轮次用尽时返回 ``PlannerResult``。

工具集
------

在 ``robots/<robot>/toolkit.py`` 里继承 ``Toolkit``，用 ``add_tool`` 注册机器\
人工具：
...
基类已注册公共文件工具；子类 ``super().__init__()`` 后追加本机器人工具即可。\
逐步状态与
...
环境和 VLA client 通常应分别继承 ``BaseEnvClient``、``BaseVLAClient``；\

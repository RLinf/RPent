...
在 RPent 中，*动作原语* 负责将一次工具调用转换为环境可执行的动作。它既可以基于 VLA、WAM 或 Diffusion Policy，也可以是 ``move_to``、
...
1. **在 primitives 中添加方法。** 在当前机器人的 primitives
   类（如 ``LiberoPrimitives``、``MyRobotPrimitives``）中添加一个方法。该方法接收工具调用的参数，执行一次或多次
...
1. **编写 ``vla_server.py``。** 该进程只持有模型权重和 CUDA 上下文。继承 :class:`rpent.robots.components.vla_facade_base.BaseVLAFacade`，实现
   ...
   ``RpcFacade.serve`` 负责绑定传输层、处理 ``healthz`` 和 ``shutdown``、检测父进程退出并清理资源。

2. **编写 model client。** 继承
   :class:`rpent.robots.components.vla_client_base.BaseVLAClient`；它已经提供公共的 ``vla.predict`` 调用，子类只需增加环境专用的输入 / 输出适配。
   LIBERO 的实现可参考
...
3. **在 primitives 中添加方法。** 在当前机器人的 primitives
   ...
   model client 的接口是
   :meth:`rpent.robots.components.pi05_vla_client.Pi05VLAClient.predict`，指令从 ``env_obs["task_descriptions"]`` 中读取；返回 ``[chunk, action_dim]``
...
- **facade 侧**：构造 ``BaseVLAFacade`` 子类时传 ``enable_sessions=True``
  和 ``session_timeout_s``，并实现 ``_on_session_drop``——session 结束（客户端调用 ``session.close`` RPC 或空闲超时）时在这里清理该客户端的策略状态。需要显式重置时，额外提供 ``reset_session`` RPC（只清策略状态，不销毁 session）。``serve`` 必须传 ``session_sweep_s`` （> 0），让后台线程定期回收过期 session。
...
大多数后端直接使用基类继承的 ``serve``：transport server 为每个请求开一个工作线程并发处理。但如果你的服务器进程用 EGL 渲染（如
...
混入你的 facade 类（**先于** ``BaseEnvFacade`` / ``BaseVLAFacade``），直接继承它覆盖的 ``serve`` 即可——它在守护线程跑 transport server，但在调用 ``serve`` 的线程（通常是主线程）串行执行每个 dispatch，通过 work
...
``session_sweep_s``）。子类**不需要**重写 ``serve`` 来委托——直接继承即可（参考 ``robots/robocasa/env_server.py`` 的
...
- **工具只返回简短的字典。** 返回值会以文本形式提供给 LLM；图像、深度数据和其他大型观测应通过 ``EnvState.save`` 保存；``EnvState`` 会把每个逻辑基础文件名自动加入其持有的 ``StepRecord.artifacts`` 集合。图像通过

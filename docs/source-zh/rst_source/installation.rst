...
- 已获权限并完成认证的 `facebook/sam3
  <https://huggingface.co/facebook/sam3>`_ checkpoint, 或本地 SAM 3.0
  ``sam3.pt`` 文件。
...
SAM3 会默认自动启动; 可设置 ``SAM3_CHECKPOINT_PATH=/path/to/sam3.pt``
使用本地 checkpoint, 或先运行一次 ``hf auth login`` 允许下载官方权重。

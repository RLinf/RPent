...
.. tip::
   ...
   ``openai-codex-cli-bin`` 目前在清华 TUNA PyPI 镜像中不可用。如果从默认 PyPI
   安装较慢，可以使用阿里云 PyPI 镜像：
...
通过 pip 安装的 Python 包不包含运行 LIBERO 所需的大型资源文件。请根据上一步安装\
的依赖组合，从以下命令中选择一条：
...
Franka 与 SO-101 的支持正在逐步接入; 每个机器人的 robot 包未来会以一个包的形式\
放在 ``robots/<name>/`` 下, 并附带 ``README.md`` 说明其 SDK /

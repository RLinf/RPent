...
memory。默认每次运行包含 3 个 planner session，每个 session 最多尝试 5 次：
...
可以通过 ``--explore-sessions`` 和 ``--explore-attempts-per-session`` 调整预算。
``reset`` 沿用环境原有的 episode reset，因此每次 reset 后规划器都需要重新感知。
探索产生的 memory 写入当前本地 inbox；除非传入 ``--no-auto-merge-memory``，否则
运行结束后会自动合并。

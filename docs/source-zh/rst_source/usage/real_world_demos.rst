...
下面三段真机演示展示 RPent 如何在任务改变、路径受阻和操作组合变化时重新观察场景并调整行动：将同一只蓝色盘子放入新的纸箱，读取瓶身和袋子标签并试抬确认抓取，再把已有的放置技能复用于钢珠倾倒。它们展示的不是固定动作的重复，而是从观察、判断、执行到纠错的闭环，以及将成功经验沉淀为可复用能力。视频为剪辑后的系统展示片段，不是评测成绩；安装配置和复现步骤请参考对应的真实机器人教程。

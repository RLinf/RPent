...
MEMORY_DIR="${MEMORY_DIR:-$REPO/examples/embodiment/primitives/workspace_pro/memory_snapshot}"
TAG=${SUITE/libero_/}_t${TASK}_s${SEED}
WORKDIR=${WORKDIR_ROOT:-/tmp}/hybrid_repl_${TAG}
OUTPUT_DIR=${OUTPUT_DIR:-$REPO/examples/embodiment/primitives/workspace_pro/results_claude_p_runs}
...
LIBERO_TYPE=$LIBERO_TYPE CUDA_VISIBLE_DEVICES=$CUDA_DEVICE \
    "$PYTHON_BIN" "$REPO/examples/embodiment/primitives/interactive_driver.py" \

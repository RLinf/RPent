"""task_t0 DOWN-approach (open BOTTOM drawer, default cabinet) SOLVE. The low bar + bowl/plate clutter make
...
def env_clear(q,grip=0.04):
    ...
    for c in range(d.ncon):
        ...
        if "pad" in rob and "cabinet" in ev: continue         # the grasp itself: allowed
        ok=False; break
...
Rg=Rdown@R.from_rotvec([0,0,np.pi/2]).as_matrix()  # gripper DOWN, finger-sep along world-y (straddle bar front/back)
...
qsol=None
seeds=[qhome]+[np.clip(qhome+np.random.uniform(-0.8,0.8,7),JL[:,0],JL[:,1]) for _ in range(70)]
for hz in [0.946,0.952,0.960,0.940,0.968,0.935]:
    for sd in seeds:
...
if qsol is None:
    ...
    imageio.mimsave(f"{OUT}/task_t0_DOWN_SOLVE.mp4", frames, fps=30, quality=8); rend.close(); env.close(); sys.exit()
setq(qsol,0.04); mujoco.mj_forward(m,d)
...
print("grasp config eef:",np.round(d.site_xpos[site],3),"(bar",HANDLE,") pads y:",round(p1[1],3),round(p2[1],3),"(straddle bar y=-0.151?)")
...
if not path:
    imageio.mimsave(f"{OUT}/task_t0_DOWN_SOLVE.mp4", frames, fps=30, quality=8); rend.close(); print("RRT FAILED"); env.close(); sys.exit()
...
imageio.mimsave(f"{OUT}/task_t0_DOWN_SOLVE.mp4", frames, fps=30, quality=8); rend.close()

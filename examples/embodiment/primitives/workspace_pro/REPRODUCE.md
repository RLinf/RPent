...
- `examples/embodiment/primitives/` — driver code (`interactive_driver.py`,
...
- `examples/embodiment/primitives/workspace_pro/hybrid_agent/` —
...
python -c "from examples.embodiment.primitives.primitives import CHECKPOINT_PATH; import os; print('Pi0 ckpt OK?', os.path.isdir(CHECKPOINT_PATH))"

# 4. (CC variant) one-cell smoke
bash examples/embodiment/primitives/workspace_pro/hybrid_agent_cc/run_one_cell.sh \
    libero_spatial_lan 0 0
# expected: ~5 min wall, audit file with libero_terminated=True

# 5. (API variant) one-cell smoke
cd examples/embodiment/primitives/workspace_pro/hybrid_agent

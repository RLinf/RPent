...
from robots.libero.robot_spec import FLASH_SUITES, _parse_config
...
def test_flash_planner_executes_native_toolkit(make_toolkit):
    toolkit, env, model = make_toolkit(molmo_client=SimpleNamespace())
    plan_dir = toolkit.memory.root / "flash"
    plan_dir.mkdir(parents=True)
    (plan_dir / "object_task_t0_plan.json").write_text(
        json.dumps({"plan": [{"action": "pi0_pick", "arguments": {"prompt": "bowl"}}]})
    )
    (plan_dir / "object_task_t0_anchors.json").write_text('{"anchors": []}')
    env.after_step = lambda: setattr(env, "terminated", True)

    result = FlashPlanner(recipe_tag="object_task_t0_s1", robot_name="libero").solve(
...
def test_native_pick_applies_flash_thresholds(
...
def test_missing_plans_reports_selected_memory_without_downloading(
